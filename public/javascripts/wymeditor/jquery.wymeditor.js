...
$.fn.wymeditor = function(options) {
  ...
  options = $.extend({
    ...
    dialogPreviewHtml: "<div class='row'>"
               		  + "<input class='wym_cancel' type='button'"
               		  + "value='{Cancel}' />"
               		  + "</div>"
					  + "<div class='wym_dialog wym_dialog_preview'"
                      + " onload='WYM_INIT_DIALOG(" + WYM_INDEX + ")'"
                      + ">"
					  + "</div>",
...
Wymeditor.prototype.html = function(html) {
  ...
  if(html) $(this._doc.body).html(html);
...
Wymeditor.prototype.toggleClass = function(sClass, jqexpr) {
  ...
  if(!$(container).attr(WYM_CLASS)) $(container).removeAttr(this._class);
...
Wymeditor.prototype.dialog = function(sType) {
  ...
  if(1) {
	...
	closeDialog();
    var e = $('<div id="dialog" class="wym_dialog"></div>'); 
 	e.html(dialogHtml); 
 	$(e).prependTo(".wym_box").hide().slideDown();
 	WYM_INIT_DIALOG(this._index); 
  }
};
...
Wymeditor.prototype.toggleHtml = function() {
  $(this._box).find(this._options.htmlSelector).slideToggle('fast');
  $(this._box).find(this._options.iframeSelector).slideToggle('fast');
};
...
Wymeditor.prototype.loadXhtmlParser = function(WymClass) {
  if(typeof XhtmlSaxListener != 'function'){
   ...
   eval($.ajax({url:this._options.basePath
...
Wymeditor.prototype.listen = function() {
  ...
  $(this._doc.body).bind("mouseup", this.mouseup);
  var images = this._doc.body.getElementsByTagName("img");
  for(var i=0; i < images.length; i++) {
    $(images[i]).bind("mouseup", this.mouseup);
  }
};
...
Wymeditor.prototype.skin = function() {
  ...
  switch(this._options.skin) {
    ...
    case WYM_DEFAULT_SKIN:
      ...
      $(this._box).addClass("wym_skin_default");
      
      //render following sections as panels
      $(this._box).find(this._options.classesSelector)
        .addClass("wym_panel");

      //render following sections as buttons
      $(this._box).find(this._options.toolsSelector)
        .addClass("wym_buttons");

      //render following sections as dropdown menus
      $(this._box).find(this._options.containersSelector)
      ...
      $(this._box).find("div.wym_area_right ul")
        .parents("div.wym_area_right").show()
        /*.parents(this._options.boxSelector)
        .find("div.wym_area_main")
        .css({"margin-right": "155px"});*/

      $(this._box).find("div.wym_area_left ul")
      ...
      $(this._box).find(".wym_section").hover(function(){ 
        $(this).addClass("hover"); 
      },function(){ 
...
function WYM_INIT_DIALOG(index) {
    ...
    var wym = window.WYM_INSTANCES[index];
    ...
    case WYM_DIALOG_LINK:
      ...
      if(selected && selected.tagName && selected.tagName.toLowerCase != WYM_A)
        selected = $(selected).parentsOrSelf(WYM_A);
    
      //fix MSIE selection if link image has been clicked
      if(!selected && wym._selected_image)
        selected = $(wym._selected_image).parentsOrSelf(WYM_A);
    break;
    ...
    if(selected) {
      $(wym._options.hrefSelector).val($(selected).attr(WYM_HREF));
      $(wym._options.srcSelector).val($(selected).attr(WYM_SRC));
      $(wym._options.titleSelector).val($(selected).attr(WYM_TITLE));
      $(wym._options.altSelector).val($(selected).attr(WYM_ALT));
    }
    
    //auto populate image fields if selected image
    if(wym._selected_image) {
      $(wym._options.dialogImageSelector + " " + wym._options.srcSelector)
        .val($(wym._selected_image).attr(WYM_SRC));
      $(wym._options.dialogImageSelector + " " + wym._options.titleSelector)
        .val($(wym._selected_image).attr(WYM_TITLE));
      $(wym._options.dialogImageSelector + " " + wym._options.altSelector)
        .val($(wym._selected_image).attr(WYM_ALT));
    }
    ...
    $(wym._options.dialogLinkSelector + " "
        ...
        if(sUrl.length > 0) {
            ...
            for(var i=0; i < nodes.length; i++) {
                if($(nodes[i]).attr(WYM_HREF) == sStamp) {
                    link = $(nodes[i]);
                    break;
                }
            }
            if(link) {
                ...
                link.attr(WYM_TITLE, $(wym._options.titleSelector).val());
            }
        }
        closeDialog();
    });
    
	console.log('IMAGE', wym._options.dialogImageSelector + " " 
		+ wym._options.submitSelector, jQuery(wym._options.dialogImageSelector + " " 
		+ wym._options.submitSelector));
    $(wym._options.dialogImageSelector + " "
        ...
        if(sUrl.length > 0) {
            ...
            for(var i=0; i < nodes.length; i++) {
                if($(nodes[i]).attr(WYM_SRC) == sStamp) {
                    image = $(nodes[i]);
                    break;
                }
            }
            if(image) {
                ...
                image.attr(WYM_TITLE, $(wym._options.titleSelector).val());
                image.attr(WYM_ALT, $(wym._options.altSelector).val());
            }
        }
        closeDialog();
    });
    ...
    $(wym._options.dialogTableSelector + " "
        ...
        if(iRows > 0 && iCols > 0) {
          ...
          var node = $(wym.findUp(wym.container(),WYM_MAIN_CONTAINERS)).get(0);
...
function closeDialog(){
	$("#dialog").slideUp('normal', function(){ $(this).remove();});
}
...
WymClassMozilla.prototype.keyup = function(evt) {
  ...
  if(evt.keyCode == 13 && !evt.shiftKey) {
    ...
    $(wym._doc.body).children(WYM_BR).remove();
...
WymClassOpera.prototype.initIframe = function(iframe) {
    ...
    $(this._box).find(this._options.toolSelector 
